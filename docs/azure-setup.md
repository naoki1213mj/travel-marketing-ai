...
| Fabric | 完了 | Fabric capacity `fcdemoeastus2001` (East US 2, F64, **Active**) を resume し、workspace `ws-3iq-demo` に Phase 9 v2 lakehouse `lh_travel_marketing_v2` (10 Delta tables in `dbo` schema) を投入済み。Data Agent v2 `Travel_Ontology_DA_v2` は Phase 10 `displayNamePropertyId` + tuned aiInstructions で best-of 12/14 grade A。`FABRIC_DATA_AGENT_URL_V2` / `FABRIC_SQL_ENDPOINT` / `FABRIC_DATA_AGENT_RUNTIME_VERSION=v2` を Container App へ反映済み。legacy `Travel_LH` は v1 rollback 用に保持 |
...
- data agent: `Travel_Ontology_DA_v2` (Phase 10 tuned)
