...
Microsoft Foundry のモデルデプロイメント設定で Content Filter を有効にする。各エージェントが gpt-5.4-mini を呼び出す際に、入出力の両方がモデル配備側で自動的にフィルタリングされる。

**ツール応答時（軽量ガード）:**
Web Search ツールの応答にもプロンプトインジェクション（間接攻撃）のリスクがある。外部データソースから受け取ったテキストをモデルへ渡す前に、同じ軽量パターン検出で明らかな注入文字列をブロックする。高度な間接攻撃対策は Foundry / AI Gateway 側のガードレール設定で補完する。

**AI Gateway 層（任意ポリシー）:**
API Management では必要に応じて `llm-content-safety` などのポリシーを追加し、モデル配備側ガードレールを補完する。
...
| AI Guardrails | モデル配備側の Content Filter / Prompt Shield と、FastAPI 側の軽量入力・ツール応答ガードの組み合わせ |
