...
| Work IQ runtime | The default runtime is `MARKETING_PLAN_RUNTIME=foundry_preprovisioned` + `WORKIQ_RUNTIME=foundry_tool`. `WorkIQMCP` exists in Foundry UI and becomes primary only with `ENABLE_WORKIQ_MCP=true` plus `X-Work-IQ-MCP-Authorization`; unavailable/auth failure falls back to legacy `WorkIQCopilot` / `mcp_M365Copilot`. Live Prompt Agents are currently synchronized to legacy only until the code deploy + env opt-in. `source_scope` is guidance/metadata, not a dynamic connector overlay. `graph_prefetch` remains the explicit rollback path using Graph Copilot Chat API (`WORK_IQ_TIMEOUT_SECONDS=120`) |
...
| `work_iq_runtime=foundry_tool` が失敗する | `MARKETING_PLAN_RUNTIME=foundry_preprovisioned`、marketing-plan Agent 同期、Foundry delegated token を確認する。`ENABLE_WORKIQ_MCP=true` の場合は browser が `X-Work-IQ-MCP-Authorization` を送っているか確認し、必要なら env を外して legacy `WorkIQCopilot` fallback で切り分ける |
