...
Work IQ は既定で **`MARKETING_PLAN_RUNTIME=foundry_preprovisioned` + `WORKIQ_RUNTIME=foundry_tool`** を使います。`postprovision.py` が Agent2 用の事前作成済み Foundry Prompt Agent を同期し、実行時はその `agent_reference` を **Foundry delegated token (`https://ai.azure.com/user_impersonation`)** 付きの Responses API で呼び出します。Foundry UI には OAuth2 の `WorkIQMCP` connection（connectionName / server_label `WorkIQMCP`, server_url `https://workiq.svc.cloud.microsoft/mcp`）を作成済みです。これは **code deploy 後に `ENABLE_WORKIQ_MCP=true` を設定し、browser が `api://workiq.svc.cloud.microsoft/WorkIQAgent.Ask` token を `X-Work-IQ-MCP-Authorization` で送った場合だけ primary** になります。未取得 / auth failure 時は legacy `WorkIQCopilot` / `mcp_M365Copilot` fallback を使います。ただし 2026-06-22 時点で request-level `WorkIQMCP` は Foundry が一貫して `500 server_error` を返すため、`ENABLE_WORKIQ_MCP=false` を既定にし、legacy `WorkIQCopilot` (OBO) を主経路にしています。live production Prompt Agents は既存 container image 互換のため、現時点では legacy `WorkIQCopilot` のみに同期済みです。postprovision は **UI で選択できる text model 全件** (`gpt-5-4-mini`, `gpt-5.4`, `gpt-4-1-mini`, `gpt-4.1`) をまとめて同期します。**`graph_prefetch` は明示 rollback** で、Microsoft Graph Copilot Chat API を per-user delegated token で呼び出して短い brief を先読みします。instructions を変えた場合は marketing-plan agent を再同期してください。
...
| Work IQ | SPA redirect URIs, Foundry delegated auth, Work IQ API SP + SPA `WorkIQAgent.Ask` delegated permission/admin grant, Graph delegated permissions for `graph_prefetch`, tenant-wide admin consent, and Microsoft 365 Copilot license verification are complete |
| Work IQ runtime | The default runtime is `MARKETING_PLAN_RUNTIME=foundry_preprovisioned` + `WORKIQ_RUNTIME=foundry_tool`. `WorkIQMCP` exists in Foundry UI and becomes primary only with `ENABLE_WORKIQ_MCP=true` plus `X-Work-IQ-MCP-Authorization`; unavailable/auth failure falls back to legacy `WorkIQCopilot` / `mcp_M365Copilot`. As of 2026-06-22, request-level `WorkIQMCP` returns `500 server_error`, so `ENABLE_WORKIQ_MCP` defaults to `false` and legacy `WorkIQCopilot` (OBO) is the primary route until Microsoft resolves it. Live Prompt Agents are currently synchronized to legacy only until the code deploy + env opt-in. `source_scope` is guidance/metadata, not a dynamic connector overlay. `graph_prefetch` remains the explicit rollback path using Graph Copilot Chat API (`WORK_IQ_TIMEOUT_SECONDS=180`, hard cap `200`) |
...
| `WORK_IQ_TIMEOUT_SECONDS` | 任意 | Foundry Work IQ tool / graph_prefetch の timeout（秒、既定 180、コード側 cap 200） |
...
Gitleaks の直近 failure は false positive です。既知 finding は `.gitleaksignore` に exact fingerprint で登録済みなので、rule を緩めず fingerprint 単位で抑止します。
...
| `work_iq_runtime=foundry_tool` が失敗する | `MARKETING_PLAN_RUNTIME=foundry_preprovisioned`、marketing-plan Agent 同期、Foundry delegated token を確認する。`ENABLE_WORKIQ_MCP=true` の request-level `WorkIQMCP` は現在 `500 server_error` の既知問題があるため、必要なら env を外して legacy `WorkIQCopilot` fallback で切り分ける |
