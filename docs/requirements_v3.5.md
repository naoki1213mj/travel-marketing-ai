...
Foundry Observability の Evaluations（GA）を使い、エージェントの品質スコアを可視化する。

**設定内容:**

- 組み込み評価器: Groundedness, Relevance, Coherence, ToolCallAccuracy, TaskAdherence
...
| 2026-03-29 | 3.5 | 付加価値機能 6 件を追加（§14）: Voice Live 音声入力、Foundry Evaluations 品質ダッシュボード、Teams 公開、Logic Apps 承認後自動アクション、Content Understanding 既存パンフレット解析、Photo Avatar + Voice Live 販促動画生成。Agent4 に AG4-05（動画生成）/ AG4-06（パンフレット参照）を追加。成果物に販促紹介動画（MP4）を追加。デモシナリオに音声入力・動画・Evaluations ダッシュボードを反映。技術スタック表に 5 サービスを追加。Out of Scope から動画生成を削除 | Team D |
