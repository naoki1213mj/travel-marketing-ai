...
| Area | Current state |
| --- | --- |
| Work IQ delegated auth | SPA redirect URIs, Foundry delegated auth, Work IQ API SP + SPA `api://workiq.svc.cloud.microsoft/WorkIQAgent.Ask` delegated permission/admin grant, Microsoft Graph permissions for the `graph_prefetch` rollback path, tenant-wide admin consent, and Microsoft 365 Copilot license verification are complete |
| Work IQ runtime | The default runtime is `MARKETING_PLAN_RUNTIME=foundry_preprovisioned` + `WORKIQ_RUNTIME=foundry_tool`. Foundry UI now has a `WorkIQMCP` OAuth2 connection (`https://workiq.svc.cloud.microsoft/mcp`, server label `WorkIQMCP`). It becomes primary only after code deploy with `ENABLE_WORKIQ_MCP=true` and browser-supplied `X-Work-IQ-MCP-Authorization`; otherwise/auth-failure falls back to legacy `WorkIQCopilot` / `mcp_M365Copilot`. As of 2026-06-22, request-level `WorkIQMCP` returns `500 server_error`, so `ENABLE_WORKIQ_MCP` defaults to `false` and legacy `WorkIQCopilot` (OBO) is the primary route until Microsoft resolves it. Live Prompt Agents are currently synchronized to legacy only to avoid breaking the deployed image. `graph_prefetch` remains the explicit rollback path using Microsoft Graph Copilot Chat API (`WORK_IQ_TIMEOUT_SECONDS=180`, hard cap `200`). Frontend preflight surfaces `auth_required`, `consent_required`, and `redirecting`, and the backend persists `work_iq_session` status so restored conversations keep the same UI state |
...
| Fabric | The current production target is workspace `ws-3iq-demo` on Fabric capacity `fcdemoeastus2001` (East US 2, F64, Active). Phase 9 lakehouse `lh_travel_marketing_v2` (10 Delta tables in `dbo`) and Phase 10 tuned Data Agent v2 `Travel_Ontology_DA_v2` are live. Agent1 prefers `FABRIC_DATA_AGENT_URL_V2` when `FABRIC_DATA_AGENT_RUNTIME_VERSION=v2`, then falls back to `FABRIC_SQL_ENDPOINT` + `FABRIC_LAKEHOUSE_DATABASE` + the configured table names, then CSV. Agent1 intentionally does not use Web Search/Web IQ; keep the Fabric-first required-tool design and data boundary. Legacy `Travel_LH` / `Travel_Ontology_DA` remain as v1 rollback only |
...
| Security scanning | Recent Gitleaks failures were false positives; `.gitleaksignore` contains exact fingerprints for the known findings |
...
| `WORK_IQ_TIMEOUT_SECONDS` | Optional | Timeout cap for the Foundry Work IQ tool (`WorkIQCopilot`/OBO) call and the `graph_prefetch` Graph Copilot Chat brief fetch (default: `180`; hard cap `200` in code) |
