...
| Work IQ runtime | The default runtime is `MARKETING_PLAN_RUNTIME=foundry_preprovisioned` + `WORKIQ_RUNTIME=foundry_tool`. Foundry UI now has a `WorkIQMCP` OAuth2 connection (`https://workiq.svc.cloud.microsoft/mcp`, server label `WorkIQMCP`). It becomes primary only after code deploy with `ENABLE_WORKIQ_MCP=true` and browser-supplied `X-Work-IQ-MCP-Authorization`; otherwise/auth-failure falls back to legacy `WorkIQCopilot` / `mcp_M365Copilot`. Live Prompt Agents are currently synchronized to legacy only to avoid breaking the deployed image. `graph_prefetch` remains the explicit rollback path using Microsoft Graph Copilot Chat API (`WORK_IQ_TIMEOUT_SECONDS=120` by default). Frontend preflight surfaces `auth_required`, `consent_required`, and `redirecting`, and the backend persists `work_iq_session` status so restored conversations keep the same UI state |
