...
| 領域 | 現在の状態 |
| --- | --- |
| Work IQ delegated auth | SPA redirect URI、Foundry delegated auth、Work IQ API SP + SPA の `api://workiq.svc.cloud.microsoft/WorkIQAgent.Ask` delegated permission/admin grant、`graph_prefetch` rollback 用 Microsoft Graph delegated permissions、tenant-wide admin consent、Microsoft 365 Copilot ライセンス確認まで完了 |
| Work IQ runtime | 既定値は `MARKETING_PLAN_RUNTIME=foundry_preprovisioned` + `WORKIQ_RUNTIME=foundry_tool` です。Foundry UI には OAuth2 の `WorkIQMCP` connection（`https://workiq.svc.cloud.microsoft/mcp`、server label `WorkIQMCP`）を作成済みです。code deploy 後に `ENABLE_WORKIQ_MCP=true` を設定し、browser が `X-Work-IQ-MCP-Authorization` を送った場合だけ primary になり、未取得 / auth failure 時は legacy `WorkIQCopilot` / `mcp_M365Copilot` fallback を使います。ただし 2026-06-22 時点で request-level `WorkIQMCP` は Foundry が一貫して `500 server_error` を返すため、`ENABLE_WORKIQ_MCP=false` を既定にし、legacy `WorkIQCopilot` (OBO) を主経路にしています。live Prompt Agents は既存 image 互換のため現時点では legacy のみに同期済みです。`graph_prefetch` は Microsoft Graph Copilot Chat API を使う明示 rollback 経路です |
...
| Security scanning | 直近の Gitleaks failure は false positive です。既知 finding は `.gitleaksignore` に exact fingerprint で登録済みです |
...
| `WORK_IQ_TIMEOUT_SECONDS` | 任意 | Foundry Work IQ tool（`WorkIQCopilot`/OBO）呼び出しと `graph_prefetch` の Graph Copilot Chat brief 取得に使う timeout 上限（既定: `180`、コード側 hard cap `200`） |
