...
| モデル配備 | テキストモデルは `gpt-5-4-mini`、`gpt-4-1-mini`、`gpt-4.1`、`gpt-5.4` を利用します。アプリの既定画像経路は `gpt-image-2` になっているため、既定名で配備するか、deployment 名が異なる場合は `GPT_IMAGE_2_DEPLOYMENT_NAME` で上書きしてください。`gpt-image-1.5` も引き続き利用可能です |
