...
def _build_work_iq_tool_guidance(
    ...
    return (
        ...
        "- KPI セクションの後ろ（末尾に Evidence 行がある場合はその直前）に、9 番目の補足セクションとして"
        "「## 社内ナレッジ反映」を追加してください。Work IQ tool の出力に明示された社内文脈だけを 2〜4 個の箇条書きで示し、"
        "各行は出所の絵文字（📧メール / 💬Teams / 📝会議メモ / 📄文書）で始め、"
        "『どの社内情報を、企画にどう反映したか』を簡潔に書きます（例: 📧 早期予約割で予約+30% の実績 → 本企画でも早割を採用）。"
        "Work IQ から該当する社内文脈が 1 件も得られなかった場合はこのセクションを省略し、"
        "推測・一般知識・ユーザー入力からの補完で埋めることは禁止です。\n"
